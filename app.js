...
const difficultSelect = document.querySelector('[difficuty-select]')
...
const completeScore = document.querySelector('[completed-score]')
const playAgain = document.querySelector('[play-again]')


// Variables
let hasUserFinishedTyping = false
let wordIndex = 0
let currentWord = ''
let hasUserComplete = false
let userScore = 0
let level = 1
let timeCountDefault = 20 //this wont change in countdown, so the countdown will hav a copy
let timeCount = 20
let wordsToType = 10


let words = ["aback","abaft","abandoned","abashed","aberrant",
    "abhorrent","abiding","abject","ablaze","able","abnormal","aboard",
    "aboriginal","abortive","abounding","abrasive","abrupt","absent","absorbed",
    "absorbing","abstracted","absurd","abundant","abusive","acceptable","accessible",
    "accidental","accurate","acid","acidic","acoustic","acrid","actually","ad hoc",
    "adamant","adaptable","addicted","adhesive","adjoining","adorable","adventurous","afraid",
    "aggressive","agonizing","agreeable","ahead","ajar","alcoholic","alert","alike","alive",
    ...
    "brash","brave","brawny","breakable","breezy","brief","bright","bright","broad","broken","brown",
    "bumpy","burly","bustling","busy","cagey","calculating","callous","calm","capable","capricious",

"careful","careless","caring","cautious","ceaseless","certain","changeable","charming","cheap","cheerful",
"chemical","chief","childlike","chilly","chivalrous","chubby","chunky","clammy","classy","clean","clear","clever",
"cloistered","cloudy","closed","clumsy","cluttered","coherent","cold","colorful","colossal","combative","comfortable",
"common","complete","complex","concerned","condemned","confused","conscious","cooing","cool","cooperative","coordinated",
"courageous","cowardly","crabby","craven","crazy","creepy","crooked","crowded","cruel","cuddly","cultured","cumbersome","curious",
"curly","curved","curvy","cut","cute","cute","cynical","daffy","daily","damaged","damaging","damp","dangerous","dapper","dark",
"dashing","dazzling","dead","deadpan","deafening","dear","debonair","decisive","decorous","deep","deeply","defeated","defective",
"defiant","delicate","delicious","delightful","demonic","delirious","dependent","depressed","deranged","descriptive","deserted",
"detailed","determined","devilish","didactic","different","difficult","diligent","direful","dirty","disagreeable","disastrous",
"discreet","disgusted","disgusting","disillusioned","dispensable","distinct","disturbed","divergent","dizzy","domineering","doubtful",
"drab","draconian","dramatic","dreary","drunk","dry","dull","dusty","dusty","dynamic","dysfunctional","eager","early","earsplitting",
"earthy","easy","eatable","economic","educated","efficacious","efficient","eight","elastic","elated","elderly","electric","elegant",
...
input.addEventListener('input', ()=>{
    ...
    let splitTyped = value.split('')

    //split them and add class
    matchWords(splitTyped)

    if(value === currentWord){ //set the user score
        userScore++
        score.textContent = userScore 
    }

    if(splitTyped.length > splitCurrent.length -1){ // the user has finished typing the current word
        moveToNextWord()
    }
})
...
function startGame(){
    hasUserComplete = false
    hideElement(welcome)
    hideElement(complete)
    hideElement(difficultSelect)
    showElement(playground, 'block')
    
    userScore = 0
    level = 1
    wordIndex = generateRandomNumber()
    hasUserFinishedTyping = false
    currentWord = words[wordIndex]
    arrangeWordToSpan(currentWord)
...
function startTime(){
    let countdownTimer = setInterval(()=>{
        if(timeCount <= 0){
            ...
            completeText.textContent = 'Time up' // if your time ends
            hasUserComplete = true
            moveToNextWord()
        }else if (hasUserComplete){
            clearInterval(countdownTimer)
        }
        else{
            time.textContent = timeCount
...
function arrangeWordToSpan(words){
    word.innerText = null
    words.split('').forEach(character =>{
        let characterSpan = document.createElement('span')
        characterSpan.classList.add('word-untyped', 'type-word')
        characterSpan.textContent = character
        word.appendChild(characterSpan)
    })
}

function matchWords(currentTyped){
    let spanWords = document.querySelectorAll('.type-word')
    
    spanWords.forEach((spanWord, index)=>{
        let characterIndex = currentTyped[index]

        if(characterIndex === undefined){
            spanWord.classList.remove('word-correct')
            spanWord.classList.remove('word-incorrect')
        }

        else if(characterIndex === spanWord.innerText){
            spanWord.classList.add('word-correct')
            spanWord.classList.remove('word-incorrect')
        }else{
            spanWord.classList.remove('word-correct')
            spanWord.classList.add('word-incorrect')
        }
    })
}
...
function moveToNextWord(){
    ...
    if(level > wordsToType  || hasUserComplete === true){ //supposed to show the user has finished typing
        hasUserComplete = true
        completeText.textContent = timeCount <= 0 ? 'Time up' : 'You completed' // if your time ends
        // completeText.textContent = 'You completed' // if your time ends

        completeScore.textContent = userScore
        timeCount = timeCountDefault + 1 // this will return the exact number cos this seems to be 1 sec late
        
        showElement(complete, 'block')
        hideElement(playground)
    }else{
        currentWord = words[wordIndex]
        arrangeWordToSpan(currentWord)
...
function showElement(domElement, property){
